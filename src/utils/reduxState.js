...
export function membersReset() {
    ...
    // 2. 특정 리듀서 상태 부분 제거
    const updatedStateWithoutMembers = { ...parsedStoredState };
    delete updatedStateWithoutMembers.members;

    // 3. 업데이트된 데이터 로컬 스토리지에 다시 저장
    localStorage.setItem('reduxState', JSON.stringify(updatedStateWithoutMembers));
}